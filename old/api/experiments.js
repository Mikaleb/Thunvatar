...
const fn__string_to_addresses = (mail_string) =>
  mail_string.replaceAll(/".*?"/g, '').split(',');
const fn__normalize_address = (mail) =>
  mail.replace(/^.*?</, '').replace(/>.*$/, '').trim();

const fn__address_to_domain = (x) => x.replace(/^.*@/, '');

const extract_unique_normalized_addresses = (addresses_string) =>
  fn__deduplicate(
    fn__string_to_addresses(addresses_string).map((x) =>
      fn__normalize_address(x),
    ),
  );
const extract_unique_domains = (addresses_string) =>
  fn__deduplicate(
    extract_unique_normalized_addresses(addresses_string).map((x) =>
      fn__address_to_domain(x),
    ),
  );
        ...
        async add(id, name, field) {
          ...
          const callbacks = {
            ...
            sender_domain: function (message) {
              return fn__address_to_domain(
                extract_unique_normalized_addresses(message.author)[0],
              );
            },

            recipients: function (message) {
              return extract_unique_normalized_addresses(
                message.recipients,
              ).join(', ');
            },
